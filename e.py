...
from mv3 import mynet
from cityscapes import CityScapes
#from testnet import mynet
from visualize import show_prediction,show_img
...
class MscEval(object):
    ...
    def evaluate(self):
        ...
        if dist.is_initialized() and not dist.get_rank()==0:
            dloader = self.dl
        for i, (imgs, label,fn,img_only_tensor) in enumerate(dloader):
            N, _, H, W = label.shape
            probs = torch.zeros((N, self.n_classes, H, W))
            probs.requires_grad = False
            imgscpu=imgs
            imgscpu_only_tensor = img_only_tensor
            imgs = imgs.cuda()
            ...
            if self.show_image==1:
                ...
                p=np.squeeze(preds)
                print(p.shape)
                colors = self.dsval.get_class_colors()
                imgscpu_only_tensor = imgscpu_only_tensor.mul(255).byte()
                image = imgscpu_only_tensor.numpy().squeeze(0).transpose((1, 2, 0))
                clean = np.zeros(lab.shape)
                
                comp_img = show_img(colors, -1, image,clean,lab,p)
                # comp_img.save(os.path.join('vis', newname))
                # img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                # plt.figure()
                # plt.imshow(img)
                cv2.imwrite(os.path.join('vis', newname), comp_img)
...
def evaluate(respth='./res1024', dspth='/home/lyf/git-repo/real-time-segmentation/datasets/cityscapes/cityscapes',e='final', istest=0,show_image=0):
    ## logger
    ...
    scope(net, input_size=(3, 1024, 1024))
    ...
    evaluator = MscEval(net, dl,dsval,istest,show_image)
...
if __name__ == "__main__":
    ...
    resepoch = '79800_2'
    setup_logger(respath, arg.epochs)
    evaluate(respath, e=arg.epochs,istest=1,show_image=0)
