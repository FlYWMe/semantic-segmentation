...
class sdBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        out1 = self.conv1x3_3(out)
        out1 = self.relu(out1)
        out1 = self.conv3x1_3(out1)
        out1 = self.bn3(out1)
...
class BasicBlockb(nn.Module):
    def __init__(self, inplanes, planes, bn_eps=1e-5,  stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                     padding=1, bias=False)
        self.bn2 = BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes, kernel_size=1, stride=stride,
                     padding=0, bias=False)
        self.bn3 = BatchNorm2d(planes)
        self.init_weight()
...
class BasicBlockc(nn.Module):
    def __init__(self, inplanes, planes, bn_eps=1e-5,  stride=1, downsample=None):
        ...
        self.conv3x1_1 = nn.Conv2d(planes, planes, (3,1), stride=1, padding=(1,0), bias=True)
        self.conv1x3_1 = nn.Conv2d(planes, planes, (1,3), stride=1, padding=(0,1), bias=True)
        self.conv3x1_2 = nn.Conv2d(planes,planes, (3,1), stride=1, padding=(1,0), bias=True)
        self.conv1x3_2 = nn.Conv2d(planes,planes,(1,3), stride=1, padding=(0,1), bias=True)
        self.init_weight()
...
def compute_speed(model, input_size, iteration):
    ...
    print('Speed Time: %.2f ms / iter    FPS: %.2f' % (elapsed_time / iteration * 1000, iteration / elapsed_time))


model=mynet(19)
compute_speed(model,(1,3,1024,1024),iteration=1000)