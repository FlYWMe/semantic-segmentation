...
class KeepRes(nn.Module):
    def __init__(self, inc, outc, *args, **kwargs):
        ...
        self.block1 = BasicBlockc(inplanes=inc, planes=outc)
        self.block2 = BasicBlockc(inplanes=inc, planes=outc)
...
class MergeAttention(nn.Module):
    ...
    def forward(self, context, highres):
        ...
        merge_at = torch.mul(merge,context_se) 
        # hr = torch.mul(merge,sp)
        return context,merge_at
    def init_weight(self):
        for ly in self.children():
            if isinstance(ly, nn.Conv2d):
                nn.init.kaiming_normal_(ly.weight, a=1)
